...
stale=0
for fn in self-service yemot-ivr admin-users; do
  if [ ! -f "fn-$fn.js" ] || [ "../supabase/functions/$fn/index.ts" -nt "fn-$fn.js" ]; then stale=1; fi
done
if [ "$stale" = 1 ]; then bash build-edge.sh; fi
run test-edge.js
exit $fail
