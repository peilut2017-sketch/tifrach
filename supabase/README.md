...
> **`yemot-ivr` מחייבת סוד.** בלעדיו הכתובת הייתה נקודת כתיבה פתוחה לכל מי שמכיר אותה,
> ולכן הפונקציה דוחה כל קריאה כל עוד הסוד לא מוגדר. לפני הפריסה:
> `supabase secrets set YEMOT_WEBHOOK_SECRET=<סיסמה-שתבחר>`
> ואז יש להעביר אותו מימות כ-`?secret=...` בכתובת ה-webhook.
> הפונקציה גם דוחה סכומים מעל ₪1,000,000 ומפסיקה לקבל תרומות אנונימיות כשתור
> הבקשות הממתינות מגיע ל-500.
...
- **הגבלת קצב ותור:** `self-service` מגבילה 10 בקשות לדקה לכל כתובת IP, וגם טופס ההצטרפות וגם עריכה עצמית מסרבות כשתור הבקשות הממתינות מגיע ל-500 — כדי שאף אחד לא יוכל להציף את עמוד האישורים. `yemot-ivr` מחייבת סוד משותף ומגבילה את סכום התרומה.
